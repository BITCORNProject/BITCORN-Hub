...
module.exports = {
	configs: {
		...
		description: 'Battelgrounds rain a certain Amount to the last 1-10 of People who were active',
		example: `$bgrain <amount> <1-${process.env.MAX_RAIN_USERS}>`,
		enabled: true,
	...
	async execute(event) {
		...
		if (cleanParams.isNumber(rain_amount) === false ||
			...
			const results = await databaseAPI.request(event.twitchId, body).bgrain();
			...
			if (results.status && results.status === 500) {
				...
				message = `DogePls SourPls You failed to summon battlegrounds rain, with your weak ass rain dance. You need to register and deposit / earn BITCORN in order to make it rain! DogePls SourPls`;
				success = true;
				...
				if (results.length > 0 && results[0].from.islocked === true) {
					...
					message = `ERROR: ${results.status || results.code} - Hmmmmm BattleGrounds Rain Fail ${event.twitchUsername} ${amount}`;
				}
			}
		}
